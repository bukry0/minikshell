...
/*   Updated: 2025/08/28 12:00:53 by bcili            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	scan_envp(t_cmddat *cmd, char *str, int id_len)
{
	...
	{
		envp_id_len = get_len_id(cmd->shell, cmd->shell->envp[i], 0);
		if (envp_id_len == id_len && !ft_strncmp(cmd->shell->envp[i], str,
				id_len))
		{
			if (ft_strchr(str, '='))
				modify_envp(cmd->shell, grbg_substr(cmd->shell,
						cmd->shell->envp[i], 0, envp_id_len),
					grbg_strdup(cmd->shell, str + envp_id_len + 1));
			break ;
		}
